    ...
    const sampleMemos = [
      ...
      },
      {
        ...
        workflow: [
          ...
          { role: 'Operations', status: 'Approved', date: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], comment: 'Stock available from preferred vendor.', signature: 'Charlie Ops' },
          { role: 'EAG', status: 'Pending' },
          { role: 'MD', status: 'Pending' },
          { role: 'Finance', status: 'Pending' },
        ],
